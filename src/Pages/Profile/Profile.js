...
import {MDBInput,MDBBtn, MDBCard, MDBCardBody,MDBCardTitle, MDBCardText, MDBCol } from 'mdbreact';
...
class Profile extends React.Component{
	...
	onChangeAvatar=(event)=>{
		...
		this.props.showToast(0,"something wrong")
	}
	}
	...
	updateUserInfo=(isUpdatedPhotoURL,downloadURL)=>{
		...
		if(isUpdatedPhotoURL){
			newinfo={
				...
				firebase.firestore().collection('users').doc(this.state.documentKey)
				.update(newinfo)
				.then(data=>{
 ...
 <MDBCol style={{ maxWidth: "26rem" }} className="mx-auto mt-4">
			...
			/>
                  ...
                  <MDBCardBody>
                    ...
                    <MDBCardText>
                    <MDBInput label="Nick Name" size="sm" value={this.state.name ? this.state.name :""}
			onChange={this.onChangeNickname}/>
                    <MDBInput label="Write yourself" size="sm" value={this.state.aboutMe ? this.state.aboutMe : ""}
			placeholder="Tell yourself.."
			onChange={this.onChangeAboutMe}/>
                    </MDBCardText>
                    <MDBBtn href="#" size="sm" onClick={this.uploadAvatar}>save</MDBBtn>
                    <MDBBtn href="#" size="sm" onClick={()=>{this.props.history.push('/chat')}}>Home</MDBBtn>
                  </MDBCardBody>
